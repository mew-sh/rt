...
| UDP remote forwarding (`rudp`) | Partial | `-L rudp://` is a real UDP listener with its own handler, multi-target selection and idle expiry. Binding the port at the far end of a chain over a SOCKS5 UDP tunnel is not implemented and is refused rather than bound locally, which would listen on the wrong host |
...
| KCP transport | Working | ARQ via the `kcp` crate, with kcp-go's crypt (13 ciphers), snappy and smux layers hand-written and checked against vectors generated by kcp-go itself. Reed-Solomon FEC generates parity on send and reconstructs from it on receive, one decoder per session; the matrix is pinned against klauspost/reedsolomon vectors. Shard counts are not auto-tuned, so a mismatch with the peer falls back to retransmission |
