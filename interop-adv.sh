...
for scheme in http socks5; do
  ...
  sleep 2
  spawn "$RT" -L "http://127.0.0.1:$cp" -F "$scheme://u1:p1@127.0.0.1:$sp"
  check "$cp" "$scheme auth  rt client -> gost server"

  sp=$((BASE++)); cp=$((BASE++))
  spawn "$RT" -L "$scheme://u1:p1@127.0.0.1:$sp"
  sleep 2
  spawn "$GOST" -L "http://127.0.0.1:$cp" -F "$scheme://u1:p1@127.0.0.1:$sp"
  check "$cp" "$scheme auth  gost client -> rt server"

  # An rt listener must reject the wrong password, not wave it through.
  sp=$((BASE++)); cp=$((BASE++))
  spawn "$RT" -L "$scheme://u1:p1@127.0.0.1:$sp"
  sleep 2
  spawn "$GOST" -L "http://127.0.0.1:$cp" -F "$scheme://u1:WRONG@127.0.0.1:$sp"
  check_denied "$cp" "$scheme auth  rt server rejects a bad password"
done
...
sleep 2
spawn "$RT" -L "http://127.0.0.1:$cp" -F "http://127.0.0.1:$h1" -F "socks5://127.0.0.1:$h2"
check "$cp" "2 hops (http then socks5)  rt client -> gost servers"

h1=$((BASE++)); h2=$((BASE++)); cp=$((BASE++))
spawn "$RT" -L "http://127.0.0.1:$h1"
spawn "$RT" -L "socks5://127.0.0.1:$h2"
sleep 2
spawn "$GOST" -L "http://127.0.0.1:$cp" -F "http://127.0.0.1:$h1" -F "socks5://127.0.0.1:$h2"
check "$cp" "2 hops (http then socks5)  gost client -> rt servers"
...
spawn "$GOST" -L "socks5://127.0.0.1:$h2"
sleep 2
spawn "$RT" -L "http://127.0.0.1:$cp" -F "http+tls://127.0.0.1:$h1" -F "socks5://127.0.0.1:$h2"
check "$cp" "2 hops (http+tls then socks5)  rt client -> gost servers"
